...
const HeaderSOSForm = ({ copilot, headerText }) => (
        ...
        if (this.state.fontsLoaded) {
            ...
            return (
                <View style={{ height, flex: 1 }}>
                    {id == 1 || id == 2 ? <TopHeader text={`SOS LEVEL-${id}`} /> :
                        <CopilotStep
...
Formats can include multiple choice questions and entering numeric values.		`}
                            order={1}
                            name="hello654654152"
                        >
                            <HeaderSOSForm headerText={this.state.formData[0]["Subcategory"]} />
                        </CopilotStep>}
