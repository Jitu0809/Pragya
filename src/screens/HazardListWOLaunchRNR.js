...
export default class HazardListWOLaunchRNR extends Component {
  state = {
    dialogVisible: false,
  ...
  _getHazardList = async () => {
    ...
    // const { navigation } = this.props.navigation.get('HazardListRNR');  
    // const state_id = navigation.getParam('state_id', null);  
    const state_id = this.props.navigation.getParam("state_id")


    return new Promise(async (res, rej) => {
      try {
        const response = await axios.post(`${API}showHazardsRNR`, {
          data: {
            sessionId,
            drtId,
            state_id
  ...
  onPressHandler = async (show, id, title, data) => {
    ...
    if (this.props.navigation.getParam("id") === 1) {
      ...
      this.setState({ dialogVisible2: show });
      this.props.navigation.navigate("ReportCategory2", {
        disaster_type_id: id,
        villageId,
      ...
      this.setState({ dialogVisible2: show });
      this.props.navigation.navigate("ReportCategory3", {
        disaster_type_id: id,
        villageId,
          ...
          >
            {dataType ? dataType.map((data, i) => {
              ...
              return (
                ...
                {
                  data.id == 22 && 
                <Text style={styles.modalText}>Locusts are short-horned grasshoppers that can form swarms containing millions of locusts, migrate long distances and cause extensive damage to crops and fodder.
                </Text>
                }
                {
                  data.id == 21 && 
               ...
               {
                  data.id == 18 && 
                <Text style={styles.modalText}>Forest fire refers to an uncontrolled fire in a forest, grassland, or other natural settings, which burns the vegetation and spreads according to wind and topography.
                </Text>
                }
                {
                  data.id == 23 && 
...
>
  {this.state.title === "GLOF" || this.state.title === "Desertification " || this.state.title === "Landslide" || this.state.title === "Flood/Cloudburst" ?
    <View>
